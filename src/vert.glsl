...
float GeomSmith(vec3 N, vec3 V, vec3 L){
    ...
    float ggx2 = GeomSchlickGGX(NdV);
    float ggx1 = GeomSchlickGGX(NdL);
...
vec3 pbr_lighting(vec3 V, vec3 L, vec3 N, vec3 albedo, vec3 radiance){
    float NdL = max(0.0, dot(N, L));
    vec3 F0 = mix(vec3(0.04), albedo, metalness);
    vec3 H = normalize(V + L);

    float NDF = DisGGX(N, H);
    float G = GeomSmith(N, V, L);
...
void main() {
	...
	vec3 lighting = pbr_lighting(V, L, normal, color, vec3(1.0));

    lighting += vec3(0.003) * color;
